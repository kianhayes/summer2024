...
declare -a solvers=('hlle hllc lhllc lhlld roe llf')

PRECURSOR='[BASH SCRIPT]: '
TEST_DIR=/data/khyaes/solvers_test
export OMP_NUM_THREADS=16
ALL_DATA_DIR=${TEST_DIR}/kh_shear
...
for solver in $solvers
    do
        ...
        cd ~/athena
        python configure.py --prob=kh -b --nscalars=1 -omp -hdf5 --flux=$solver --hdf5_path=$HDF5_DIR --include=/usr/include/openmpi-x86_64/ > ${ALL_DATA_DIR}/${solver}_configuration_output.txt
        ...
        if [ $? != 0 ]; then 
            ...
            mkdir $DATA_DIR
            mkdir $PLOTS_DIR
            mv ${ALL_DATA_DIR}/${solver}_configuration_output.txt $SOLVER_DIR
            make clean > /dev/null
        ...
        ~/athena/bin/athena -i /data/khyaes/solvers_test/athinput.kh-shear-lecoanet > ${SOLVER_DIR}/${solver}_simulation_output.txt
        ...
        python /home/khayes/summer2024/scripts/plot_all_data.py > ${SOLVER_DIR}/${solver}_plot_output.txt 2>&1

        ffmpeg -r 25 -i ${PLOTS_DIR}/%1d.png ../${solver}_video.mp4 > ${SOLVER_DIR}/${solver}_ffmpeg_output.txt 2>&1
